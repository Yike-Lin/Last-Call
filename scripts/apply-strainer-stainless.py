...
INPUT_GLB = r"D:\Code\Last Call\output\blender\过滤器.glb"
OUTPUT_BLEND = r"D:\Code\Last Call\output\blender\hawthorne_strainer_stainless.blend"
OUTPUT_GLB = r"D:\Code\Last Call\output\blender\hawthorne_strainer_stainless.glb"
OUTPUT_PREVIEW = r"D:\Code\Last Call\output\blender\hawthorne-strainer-stainless-preview.png"
...
def make_stainless_material():
    ...
    mapping.name = "Fine Brushing Scale"
    mapping.location = (-500, 0)

    wave = nodes.new("ShaderNodeTexWave")
    wave.name = "Fine Brushing"
...
def add_studio_preview(meshes):
    ...
    bpy.ops.mesh.primitive_plane_add(size=8, location=(0, 0, -0.09))
    ...
    def add_area(name, location, energy, size, color):
        ...
        look_at(lamp, (-0.08, 0.03, 0.0))
        return lamp

    add_area("__Key Softbox", (1.8, -2.2, 2.6), 780, 2.4, (1.0, 0.78, 0.56))
    add_area("__Cool Rim", (-1.8, 1.5, 1.8), 560, 2.0, (0.47, 0.7, 1.0))
    add_area("__Top Strip", (0.0, 0.4, 3.1), 440, 1.6, (1.0, 0.92, 0.78))

    camera_data = bpy.data.cameras.new("__Studio Camera")
    camera = bpy.data.objects.new("__Studio Camera", camera_data)
    studio.objects.link(camera)
    camera.location = (0.75, -1.45, 1.35)
    camera_data.lens = 58
    camera_data.sensor_width = 36
    look_at(camera, (-0.08, 0.03, 0.0))
...
def remove_inner_cylinder_band(obj):
    """Remove the merged inner sleeve while preserving the helical spring."""
    bm = bmesh.new()
    ...
    for face in bm.faces:
        ...
        broad_band = (
            0.16 < radius < 0.285
            and -0.055 < center.z < 0.045
            and normal_z > 0.35
        )
        lower_band = 0.11 < radius < 0.235 and center.z < -0.015
        if broad_band or lower_band:
...
def main():
    ...
    if not meshes:
        raise RuntimeError("No mesh objects found in filter GLB")

    meshes = [obj for obj in bpy.context.scene.objects if obj.type == "MESH"]
    material = make_stainless_material()
    removed = []
    spring = bpy.data.objects.get("tripo_part_3")
    if spring is not None:
        deleted_faces = remove_inner_cylinder_band(spring)
        removed.append(f"tripo_part_3_inner_cylinder_band_faces:{deleted_faces}")
    ...
    if root:
        root["asset_type"] = "hawthorne_strainer"
        root["material_profile"] = "LC_Brushed_Stainless_Steel"
        root["removed_components"] = ",".join(removed)
    ...
    print("STRAINER_STAINLESS_RESULT", {
        "blend": OUTPUT_BLEND,
        "glb": OUTPUT_GLB,
