      ...
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="panel flex flex-col gap-5 p-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="max-w-2xl text-sm leading-7 text-[var(--muted)]">{recipe.summary}</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="panel p-4">
              <p className="text-xs uppercase tracking-wide text-[var(--muted)]">杯型</p>
              <p className="mt-2 text-base font-semibold">{recipe.glassware}</p>
            </div>
            <div className="panel p-4">
              <p className="text-xs uppercase tracking-wide text-[var(--muted)]">做法</p>
              <p className="mt-2 text-base font-semibold">{recipe.method}</p>
            </div>
            <div className="panel p-4">
              <p className="text-xs uppercase tracking-wide text-[var(--muted)]">难度</p>
              <p className="mt-2 text-base font-semibold">{recipe.difficulty}</p>
            </div>
            <div className="panel p-4">
              <p className="text-xs uppercase tracking-wide text-[var(--muted)]">预计酒精度</p>
          ...
          <ul className="space-y-3 text-sm text-[var(--muted)]">
            {recipe.ingredients.map((ingredient) => (
              <li key={ingredient.name} className="flex items-center justify-between gap-4 border-b border-white/5 pb-3 last:border-b-0 last:pb-0">
                ...
                <strong className="font-medium text-[var(--foreground)]">{ingredient.amount}</strong>
              </li>
            ))}
          </ul>
        </div>
          ...
          <ol className="space-y-3 text-sm leading-6 text-[var(--muted)]">
            {recipe.steps.map((step) => (
              <li key={step} className="flex gap-3">
                <span className="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full border border-white/10 text-xs font-semibold text-[var(--foreground)]">
